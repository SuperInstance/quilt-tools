...
export async function evaluateRouter(cell, ctx, input, runtime) {
    ...
    for (const rule of cell.def.rules) {
        if (evalWhen(rule.when, ctx)) {
            ...
            // PLAY-TEST FIX: pass the caller's context through to the delegated
            // cell. Previously runtime.call dropped it, so every caller shared one
            // per-context cache entry on the delegated cell (tenant isolation and
            // caller-aware downstream routing both collapsed).
